...
const UserReport = () => {
  ...
  const upCookieCount = () => {
    ...
    document.cookie =
      `timesVisited=${++int}; expires=` + new Date(2023, 0, 1).toUTCString();
    setTimesVisited(int);
  };
  ...
  //this will run when the page loads and either generate the first timesVisited cookie if one doesn't exist yet or increase the count by one if it does exist.
  useEffect(() => {
    if (
      !document.cookie
        .toString()
